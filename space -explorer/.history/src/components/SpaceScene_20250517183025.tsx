...
const CameraZoom: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedPlanet && planetPosition) {
      // Zoom camera to selected planet (offset 5 units away on z-axis)
  ...
  return (
    <Canvas camera={{ position: [0, 20, 35], fov: 60 }} style={{ background: '#000000' }}>
      ...
      <OrbitControls
        ref={controlsRef}
        enablePan={false}
        enableZoom={true}          // Zoom always enabled
        enableRotate={!selectedPlanet}  // Disable rotation when a planet is selected
        minDistance={10}
        maxDistance={100}
      />
    </Canvas>
