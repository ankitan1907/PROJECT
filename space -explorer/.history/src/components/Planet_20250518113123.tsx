...
const Planet: React.FC<PlanetProps> = ({
  ...
  // Semi-major and semi-minor axes for ellipse
  const a = distance; // semi-major axis
  const e = eccentricity;
  const b = a * Math.sqrt(1 - e * e); // semi-minor axis
  const c = a * e; // focal distance — shift for the Sun at one focus

  // Matrix to tilt the orbit by inclination angle
  const inclinationMatrix = useMemo(() => {
    return new THREE.Matrix4().makeRotationX(inclination);
  }, [inclination]);

  // Orbit path geometry - shifted by -c to have Sun at origin
  const orbitGeometry = useMemo(() => {
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      // Shift ellipse center by -c so Sun is at focus (0,0)
  ...
  useFrame((state, delta) => {
    if (planetRef.current && !isSelected) {
      ...
      // Elliptical position before tilt, shifted by -c
