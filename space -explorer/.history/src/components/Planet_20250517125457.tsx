...
const Planet: React.FC<PlanetProps> = ({
  ...
  if (orbitRef.current && planetRef.current && !isSelected) {
    // Calculate the initial angle offset based on current time (seconds)
    const elapsed = state.clock.getElapsedTime();

    // Calculate the starting angle to scatter planets according to real time
    const initialAngle = (elapsed * orbitalSpeed) % (2 * Math.PI);
