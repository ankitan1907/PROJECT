...
const Planet: React.FC<PlanetProps> = ({
  ...
  const c = a * e; // Focus offset (distance from center to focus)

  // Matrix to rotate orbit plane by inclination angle
  const inclinationMatrix = useMemo(() => {
    return new THREE.Matrix4().makeRotationX(inclination);
  }, [inclination]);

  // Compute points along the elliptical orbit path
  const orbitPoints = useMemo(() => {
    const points: THREE.Vector3[] = [];
    const segments = 128;
    for (let i = 0; i <= segments; i++) {
      const theta = (i / segments) * Math.PI * 2;
      const x = a * Math.cos(theta) - c;  // << Corrected here
  ...
  useFrame((state, delta) => {
    if (planetRef.current && !isSelected) {
      ...
      const x = a * Math.cos(angle) - c;  // << Corrected here
  ...
  // Create ring geometry if ring texture exists
