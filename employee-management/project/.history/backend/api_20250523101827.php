...
// CORS headers - allow all origins, methods and headers required for your app
...
if ($method === 'POST') {
    ...
    if ($data && isset($data['code'], $data['fullName'], $data['designation'], $data['qualification'])) {
        ...
        // Prepare values for query with NULL handling
        $subDesignationVal = (empty($subDesignation)) ? 'NULL' : "'$subDesignation'";
        $subDesignation2Val = (empty($subDesignation2)) ? 'NULL' : "'$subDesignation2'";
        $hindiKnowledgeVal = (empty($hindiKnowledge)) ? "'Null'" : "'$hindiKnowledge'";
