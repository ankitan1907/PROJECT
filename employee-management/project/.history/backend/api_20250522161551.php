...
if ($method === 'POST') {
    ...
    if ($data && isset($data['code']) && isset($data['fullName']) && isset($data['designation']) && isset($data['qualification'])) {
        ...
        $subDesignation = $conn->real_escape_string($data['subDesignation'] ?? '');
        $subDesignation2 = $conn->real_escape_string($data['subDesignation2'] ?? '');
        $hindiKnowledge = $conn->real_escape_string($data['hindiKnowledge'] ?? 'Null');
        ...
        $sql = "INSERT INTO employees (code, fullName, designation, subDesignation, subDesignation2, hindiKnowledge, qualification)
                VALUES ('$code', '$fullName', '$designation', '$subDesignation', '$subDesignation2', '$hindiKnowledge', '$qualification')";
